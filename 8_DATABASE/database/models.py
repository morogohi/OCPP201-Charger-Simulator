"""
제주 EV 충전기 데이터 관리 시스템 - SQLAlchemy ORM 모델
...
class ChargerInfo(Base):
    ...
    station_id = Column(String(50), ForeignKey('station_info.station_id'), nullable=False, comment="소속 충전소 ID")
    ...
    last_status_update = Column(DateTime, default=datetime.utcnow, comment="상태 마지막 업데이트")
    ...
    last_maintenance = Column(DateTime, nullable=True, comment="마지막 정비일시")
    next_maintenance = Column(DateTime, nullable=True, comment="다음 정비 예정일시")
...
class ChargerUsageLog(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_usage_charger_date', 'charger_id', 'session_date'),
    )
    
    id = Column(Integer, primary_key=True, autoincrement=True)
    charger_id = Column(String(50), ForeignKey('charger_info.charger_id'), nullable=False, comment="충전기 ID")
    ...
    # 시간 정보
    start_time = Column(DateTime, nullable=False, comment="충전 시작 시간")
    end_time = Column(DateTime, nullable=True, comment="충전 종료 시간")
    ...
    payment_date = Column(DateTime, nullable=True, comment="결제 일시")
    ...
    # 추가 데이터
    extra_data = Column(JSON, nullable=True, comment="추가 데이터 (JSON)")
    
    created_at = Column(DateTime, default=datetime.utcnow, comment="기록 생성일시")
...
class PowerConsumption(Base):
    ...
    # 시간 정보
    measurement_time = Column(DateTime, nullable=False, comment="측정 시간")
...
class DailyChargerStats(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_daily_charger_date', 'stats_date'),
    )
    ...
    # 시간대별 통계 (JSON)
    hourly_energy = Column(JSON, nullable=True, comment="시간대별 에너지 (JSON)")
    hourly_sessions = Column(JSON, nullable=True, comment="시간대별 세션 수 (JSON)")
    hourly_revenue = Column(JSON, nullable=True, comment="시간대별 매출 (JSON)")
...
class HourlyChargerStats(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    charger_id = Column(String(50), ForeignKey('charger_info.charger_id'), nullable=False, comment="충전기 ID")
    stats_hour = Column(DateTime, nullable=False, comment="통계 시간 (HH:00:00)")
...
class StationDailyStats(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_station_daily_date', 'stats_date'),
    )
...
class DatabaseManager:
    ...
    def __init__(self, database_url: str = "sqlite:///./charger_management.db"):
        """
        데이터베이스 초기화
...
# 기본 데이터베이스 URL (SQLite)
DEFAULT_DATABASE_URL = "sqlite:///./charger_management.db"
